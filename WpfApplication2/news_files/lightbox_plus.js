...
Lightbox.prototype = {
	...
	_createBoxOn : function(obj,option)
	{
		...
		Spica.Event.register(img,'click',function(evt) { self._close_next(evt); });    		//self._close(evt); });
		...
		//caption.innerHTML = 'tets';      //my
		//caption.style.display = 'block'; //my
	...
	_close_next :function(evt){
		 var self = this;

		var check = self._current_setindex();
		if (check > -1)
		{
			if (check < self._get_setlength() - 1) {	self._show_next(+1);	}
				else
						{  self._close(evt);   }		}	},
	...
	_show_action : function()
	{
		...
		if (check > -1)
		{
			...
			//	var img_t = document.getElementById('lightboxImage');
            //    Spica.Event.register(img_t,'click',function() { self._show_next(+1); });
			//} else {
	...
	_prepare : function(num)
	{
		...
		for (var i in objs)
		{
			...
			//if (obj) obj.style.display = 'none'; //my
		}
	},
	_show : function(num)
	{
	...
	_doResizing : function()
	{
		var self = this;
		var diff = {
			x: ( self._anim.f ) ? Math.floor((self._anim.w - self._img.width) / 5) : 0,
			y: ( self._anim.f ) ? Math.floor((self._anim.h - self._img.height) / 5) : 0
		};
...
Spica.Event.run(function() {
	var lightbox = new Lightbox({
		...
		effectimg:'/lightbox_plus/new.gif',
		effectpos:{x:-40,y:-20},
